...
// find process from process_map using the cr3 as search key, then find module from proc->module_list using eip(pc) as search key
...
// find process from process_pid_map using pid and return proc->cr3
int vmi_find_cr3_by_pid(uint32_t pid);

// find process from process_map using cr3/pgd and return proc->pid 
...
// find process name and pid from process_map using cr3 as search key
int vmi_find_procname_pid_by_cr3(uint32_t cr3, char proc_name[], size_t len, uint32_t *pid);

// find process name and cr3 from process_pid_map using the pid as search key
...
int vmi_get_proc_modules(uint32_t pid, uint32_t mod_no, modinfo_t *buf);
...
extern decaf_target_ulong VMI_guest_kernel_base;
