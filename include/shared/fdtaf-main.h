...
extern gpa_t fdtaf_get_phys_addr(CPUState *cs, gva_t addr);

// Convert virtual address into physical address for given cr3 - cr3 is a phys addr
// The implementation is target-specific
// extern gpa_t FDTAF_get_physaddr_with_cr3(CPUState *cs, fdtaf_target_ulong cr3, gva_t addr);
// defined in target/(i386,arm,mip)/..., TODO
extern gpa_t fdtaf_get_phys_addr_with_pgd(CPUState *cs, gpa_t pgd, gva_t addr);

// wrapper -- pgd is the generic term while cr3 is the register in x86
#define fdtaf_get_phys_addr_with_cr3(cs, _pgd, _addr) fdtaf_get_phys_addr_with_pgd(cs, _pgd, _addr)

/***************************************************************************************************************/
// The basic functions for reading/writing mem, which are used by some following functions
extern fdtaf_errno_t fdtaf_memory_rw(CPUState* cs, fdtaf_target_ulong addr, void *buf, int len, int is_write);

fdtaf_errno_t fdtaf_memory_rw_with_pgd(
    CPUState *cs,
    fdtaf_target_ulong pgd,
    gva_t addr,
    void *buf,
    int len,
    int is_write);
...
/***************************************************************************************************************/
// Encapsulate fdtaf_memory_rw and fdtaf_memory_rw_with_pgd for ease of use
...
extern fdtaf_errno_t fdtaf_read_mem(CPUState *cs, gva_t vaddr, int len, void *buf);
...
/// If failure, it usually means that the given virtual address cannot be converted
/// into physical address. It could be either invalid address or swapped out.
extern fdtaf_errno_t fdtaf_write_mem(CPUState *cs, gva_t vaddr, int len, void *buf);

extern fdtaf_errno_t fdtaf_read_mem_with_pgd(CPUState *cs, fdtaf_target_ulong pgd, gva_t vaddr, int len, void *buf);
extern fdtaf_errno_t fdtaf_write_mem_with_pgd(CPUState *cs, fdtaf_target_ulong pgd, gva_t vaddr, int len, void *buf);
fdtaf_errno_t fdtaf_read_ptr(CPUState *env, gva_t vaddr, gva_t *pptr);
/***************************************************************************************************************/

// For keylogger plugin
extern void * FDTAF_KbdState;
extern void fdtaf_keystroke_read(uint8_t taint_status);
extern void fdtaf_keystroke_place(int keycode);
...
int fdtaf_bdrv_pread(void *opaque, int64_t offset, void *buf, int count);
...
// fdtaf_flush_translation_cache is a wrapper for tb_flush that iterates through all of
...
extern void fdtaf_flush_translation_page(CPUState *cs, gva_t pc);

// Register block/page/cache-level cache to be flushed to a single linked list named
// flush_list_internal(defined in fdtaf-main-internal) with type and addr
extern void register_fdtaf_flush_translation_cache(int type, gva_t pc);

// Do flush
extern void fdtaf_perform_flush(CPUState *cs);
/***************************************************************************************************************/
