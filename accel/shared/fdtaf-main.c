...
// fdtaf_errno_t fdtaf_memory_rw(CPUState* cs, fdtaf_target_ulong addr, void *buf, int len, int is_write)
// {
...
// 		phys_addr = fdtaf_get_phys_addr(cs, page);
...
{
    CPU_FOREACH(cpu) {
            fdtaf_flush_translation_block_fast(cs, pc)
        }
    }
}
...
{
    CPU_FOREACH(cpu) {
            fdtaf_flush_translation_page_fast(cs, pc)
        }
    }
}
#endif
...
void fdtaf_bdrv_open(int index, void *opaque)
{
    unsigned long img_size = ((BlockDriverState *)opaque)->total_sectors * 512;

    if(!qemu_pread)
        qemu_pread = (qemu_pread_t)fdtaf_bdrv_pread;
...
void fdtaf_nic_in(uint32_t addr, int size)
{
// #ifdef CONFIG_TCG_TAINT
	// CPUState cs = fdtaf_get_current_cpu();
    // cpu_synchronize_state(cs);
    // CPUArchState *env = (CPUArchState *)cs->env_ptr;
...
void fdtaf_nic_out(const uint32_t addr, const int size)
{
// #ifdef CONFIG_TCG_TAINT
	// CPUState cs = fdtaf_get_current_cpu();
    // cpu_synchronize_state(cs);
...
// void fdtaf_read_keystroke(void *s);
// void fdtaf_after_loadvm(const char *); 
// void fdtaf_update_cpl(int cpl);
// void fdtaf_after_iret_protected(void);
// void fdtaf_loadvm(void *opaque);
...
// {
//     uint32_t thread_info = fdtaf_get_esp(cs) & ~ (GUEST_OS_THREAD_SIZE - 1);
