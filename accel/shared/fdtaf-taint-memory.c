...
static void taint_data_flow(ram_addr_t paddr, unsigned size, uint32_t is_load)
{
    FILE *taint_log_fp;
    ...
    pc_base = fdtaf_get_pc(cs);
    if (pc_base != last_pc_base)
    {
        last_pc_base = pc_base;
        current_pgd = fdtaf_get_pgd(cs);
...
static void taint_ld_mmu_internal(CPUArchState *env, gva_t vaddr, MemOpIdx oi)
{
    ...
    paddr = fdtaf_get_phys_addr(cs, vaddr);
    ...
    if (size > 1 && unlikely((paddr & ~TARGET_PAGE_MASK) + size - 1 >= TARGET_PAGE_SIZE))
    {
        ...
        next_page_paddr = fdtaf_get_phys_addr(cs, next_page_vaddr);
    ...
    if (memcmp(env->taint_temps, zero_index, size) && fdtaf_is_callback_needed(FDTAF_READ_TAINTMEM_CB))
    {
        helper_fdtaf_invoke_read_taint_mem(vaddr, paddr, size, env->taint_temps);
    }
    return;
}
#endif
...
static void taint_st_mmu_internal(CPUArchState *env, gva_t vaddr, MemOpIdx oi)
{
    ...
    if (size > 1 && unlikely((paddr & ~TARGET_PAGE_MASK) + size - 1 >= TARGET_PAGE_SIZE))
    {
        ...
        if ((is_tainted || changed) && fdtaf_is_callback_needed(FDTAF_WRITE_TAINTMEM_CB))
        {
            helper_fdtaf_invoke_write_taint_mem(vaddr, paddr, size, env->taint_temps);
        }
    }
