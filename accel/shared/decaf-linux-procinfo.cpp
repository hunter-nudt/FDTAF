...
#define FILL_decaf_target_ulong_FIELD(field) pi.field = pt.get(sSectionNum + #field, INVALID_VAL)
void load_one_section(const boost::property_tree::ptree &pt, int iSectionNum, ProcInfo &pi)
{
    ...
    const decaf_target_ulong INVALID_VAL = -1;

    // fill other fields
    FILL_decaf_target_ulong_FIELD(init_task_addr  );
    FILL_decaf_target_ulong_FIELD(init_task_size  );
    FILL_decaf_target_ulong_FIELD(ts_tasks        );
    FILL_decaf_target_ulong_FIELD(ts_pid          );
    FILL_decaf_target_ulong_FIELD(ts_tgid         );
    FILL_decaf_target_ulong_FIELD(ts_group_leader );
    FILL_decaf_target_ulong_FIELD(ts_thread_group );
    FILL_decaf_target_ulong_FIELD(ts_real_parent  );
    FILL_decaf_target_ulong_FIELD(ts_mm           );
    FILL_decaf_target_ulong_FIELD(ts_stack        );
    FILL_decaf_target_ulong_FIELD(ts_real_cred    );
    FILL_decaf_target_ulong_FIELD(ts_cred         );
    FILL_decaf_target_ulong_FIELD(ts_comm         );

	FILL_decaf_target_ulong_FIELD(modules         );
	FILL_decaf_target_ulong_FIELD(module_name     );
	FILL_decaf_target_ulong_FIELD(module_init     );
	FILL_decaf_target_ulong_FIELD(module_size     );
	FILL_decaf_target_ulong_FIELD(module_list     );
			    
    FILL_decaf_target_ulong_FIELD(cred_uid        );
    FILL_decaf_target_ulong_FIELD(cred_gid        );
    FILL_decaf_target_ulong_FIELD(cred_euid       );
    FILL_decaf_target_ulong_FIELD(cred_egid       );
    FILL_decaf_target_ulong_FIELD(mm_mmap         );
    FILL_decaf_target_ulong_FIELD(mm_pgd          );
    FILL_decaf_target_ulong_FIELD(mm_arg_start    );
    FILL_decaf_target_ulong_FIELD(mm_start_brk    );
    FILL_decaf_target_ulong_FIELD(mm_brk          );
    FILL_decaf_target_ulong_FIELD(mm_start_stack  );
    FILL_decaf_target_ulong_FIELD(vma_vm_start    );
    FILL_decaf_target_ulong_FIELD(vma_vm_end      );
    FILL_decaf_target_ulong_FIELD(vma_vm_next     );
    FILL_decaf_target_ulong_FIELD(vma_vm_file     );
    FILL_decaf_target_ulong_FIELD(vma_vm_flags    );
    FILL_decaf_target_ulong_FIELD(vma_vm_pgoff    );
    FILL_decaf_target_ulong_FIELD(file_dentry     );
	FILL_decaf_target_ulong_FIELD(file_inode		);
    FILL_decaf_target_ulong_FIELD(dentry_d_name   );
    FILL_decaf_target_ulong_FIELD(dentry_d_iname  );
    FILL_decaf_target_ulong_FIELD(dentry_d_parent );
    FILL_decaf_target_ulong_FIELD(ti_task         );
	FILL_decaf_target_ulong_FIELD(inode_ino);

	FILL_decaf_target_ulong_FIELD(proc_fork_connector);
	FILL_decaf_target_ulong_FIELD(proc_exit_connector);
	FILL_decaf_target_ulong_FIELD(proc_exec_connector);
	FILL_decaf_target_ulong_FIELD(vma_link);
	FILL_decaf_target_ulong_FIELD(remove_vma);
	FILL_decaf_target_ulong_FIELD(vma_adjust);

	FILL_decaf_target_ulong_FIELD(trim_init_extable);
#ifdef TARGET_MIPS
    FILL_decaf_target_ulong_FIELD(mips_pgd_current);
#endif
}

// find the corresponding section for the current os and return the section number
int find_match_section(const boost::property_tree::ptree &pt, decaf_target_ulong tulInitTaskAddr)
{
    ...
    for(int i = 1; i<=cntSection; ++i)
    {
        ...
        decaf_target_ulong tulAddr = pt.get<decaf_target_ulong>(sSectionNum + ".init_task_addr");
...
private:
    ...
    void load_cur_section()
    {
        ...
        BOOST_FOREACH(boost::property_tree::ptree::value_type &v, *m_cur_section)
        {
            ...
            // insert function
            // decaf_target_ulong addr = m_cur_section->get<decaf_target_ulong>(v.first);
