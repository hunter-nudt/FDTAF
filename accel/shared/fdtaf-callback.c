...
typedef struct callback_struct{
	...
	fdtaf_callback_func_t callback;
    ...
    OCB_t type)
{
    ...
    switch (type)
    {
        ...
        case (OCB_ALL):
        {
            ...
            if (enableAllBlockBeginCallbacksCount == 1)
            {
                ...
                register_fdtaf_flush_translation_cache(ALL_CACHE, 0);
            }
            break;
        }
        // case (OCB_CONST):
        ...
        //     {
        //         register_fdtaf_flush_translation_cache(BLOCK_LEVEL, addr);
        //     }
        //     break;
        // }
        case (OCB_CONST_NOT):
        ...
        case (OCB_PAGE):
        {
            ...
            if (counting_hashtable_add(pOBBPageTable, addr) == 1) {
                register_fdtaf_flush_translation_cache(PAGE_LEVEL, addr);
            }
            break;
        }
    }
    ...
    return ((fdtaf_handle)cb_struct);
}

fdtaf_errno_t fdtaf_unregister_optimized_block_begin_callback(fdtaf_handle handle)
{
	...
	QLIST_FOREACH_SAFE(cb_struct, &callback_list_heads[FDTAF_BLOCK_BEGIN_CB], link, cb_next) {
		if((fdtaf_handle)cb_struct != handle)
			continue;
		...
		switch(cb_struct->ocb_type)
		{
            ...
            //     {
            //         register_fdtaf_flush_translation_cache(BLOCK_LEVEL, cb_struct->from);
            //     }
            //     break;
            // }
			case (OCB_PAGE):
            {
                ...
                {
                    register_fdtaf_flush_translation_cache(PAGE_LEVEL, cb_struct->from);
                }
                break;
            }
		}
    ...
    int cb_cond)
{
    ...
    {
        return(fdtaf_register_optimized_block_begin_callback(cb_func, cb_cond, INV_ADDR, OCB_ALL));
    }
...
int fdtaf_unregister_callback(fdtaf_callback_type_t cb_type, fdtaf_handle handle)
{
    if (cb_type == FDTAF_BLOCK_BEGIN_CB)
    {
        return (fdtaf_unregister_optimized_block_begin_callback(handle));
    }
...
int fdtaf_is_callback_needed(fdtaf_callback_type_t cb_type)
{
    return !QLIST_EMPTY(&callback_list_heads[cb_type]);
}

int fdtaf_is_callback_needed_for_opcode(int op)
{
...
int fdtaf_is_block_begin_callback_needed(gva_t pc)
{
...
void helper_fdtaf_invoke_block_begin_callback(CPUState* cs, TranslationBlock* tb)
{
    static callback_struct *cb_struct, *cb_next;
    static fdtaf_callback_params params;
    ...
    QLIST_FOREACH_SAFE(cb_struct, &callback_list_heads[FDTAF_BLOCK_BEGIN_CB], link, cb_next){
        ...
        if(cb_struct->enabled){
            params.cbhandle = (fdtaf_handle)cb_struct;
            switch (cb_struct->ocb_type)
            {
                default:
                case (OCB_ALL):
...
void helper_fdtaf_invoke_block_end_callback(CPUState *cs, TranslationBlock *tb, gva_t from)
{
    static callback_struct *cb_struct, *cb_next;
    static fdtaf_callback_params params;
...
void helper_fdtaf_invoke_insn_before_callback(CPUState *cs, uint32_t pc, uint32_t mips_opcode)
{
	static callback_struct *cb_struct, *cb_next;
	static fdtaf_callback_params params;
...
void helper_fdtaf_invoke_insn_after_callback(CPUState *cs)
{
	static callback_struct *cb_struct, *cb_next;
    static fdtaf_callback_params params;
...
void helper_fdtaf_invoke_eip_check_callback(gva_t source_eip, gva_t target_eip, gva_t target_eip_taint)
{
	static callback_struct *cb_struct, *cb_next;
	static fdtaf_callback_params params;
...
void fdtaf_invoke_tlb_exec_callback(CPUState *cs, gva_t vaddr)
{
    callback_struct *cb_struct, *cb_next;
    fdtaf_callback_params params;
...
void helper_fdtaf_invoke_nic_rec_callback(uint8_t * buf, int size, int cur_pos, int start, int stop)
{
	static callback_struct *cb_struct, *cb_next;
	static fdtaf_callback_params params;
	params.nr.buf = buf;
...
void helper_fdtaf_invoke_nic_send_callback(uint32_t addr, int size, uint8_t *buf)
{
	static callback_struct *cb_struct, *cb_next;
	static fdtaf_callback_params params;
	params.ns.addr = addr;
...
void helper_fdtaf_invoke_mem_read_callback(gva_t vaddr, ram_addr_t paddr, unsigned long value, DATA_TYPE data_type)
{
    static callback_struct *cb_struct, *cb_next;
    static fdtaf_callback_params params;
...
void helper_fdtaf_invoke_mem_write_callback(gva_t vaddr, ram_addr_t paddr, unsigned long value, DATA_TYPE data_type)
{
	static callback_struct *cb_struct, *cb_next;
	static fdtaf_callback_params params;
...
void helper_fdtaf_invoke_keystroke_callback(int keycode, uint32_t *taint_mark)
{
	static callback_struct *cb_struct, *cb_next;
	static fdtaf_callback_params params;
...
void helper_fdtaf_invoke_read_taint_mem(gva_t vaddr, ram_addr_t paddr, uint32_t size, uint8_t *taint_info)
{
	static callback_struct *cb_struct, *cb_next;
	static fdtaf_callback_params params;
...
void helper_fdtaf_invoke_write_taint_mem(gva_t vaddr, ram_addr_t paddr, uint32_t size, uint8_t *taint_info)
{
	static callback_struct *cb_struct, *cb_next;
	static fdtaf_callback_params params;
