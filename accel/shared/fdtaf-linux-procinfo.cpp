...
gva_t find_taskstruct_from_threadinfo(CPUState * cs, gva_t threadinfo, ProcInfo* pPI)
{
    ...
    if (is_kernel_address(threadinfo))
    {
        ...
        for (i = 0; i < MAX_THREAD_INFO_SEARCH_SIZE; i+= sizeof(target_ptr))
        {
            temp = (threadinfo + i);
            candidate = 0;
            fdtaf_read_ptr(cs, temp, &candidate);
...
#define FILL_fdtaf_target_ulong_FIELD(field) pi.field = pt.get(sSectionNum + #field, INVALID_VAL)
void load_one_section(const boost::property_tree::ptree &pt, int iSectionNum, ProcInfo &pi)
{
    ...
    const fdtaf_target_ulong INVALID_VAL = -1;

    // fill other fields
    FILL_fdtaf_target_ulong_FIELD(init_task_addr  );
    FILL_fdtaf_target_ulong_FIELD(init_task_size  );
    FILL_fdtaf_target_ulong_FIELD(ts_tasks        );
    FILL_fdtaf_target_ulong_FIELD(ts_pid          );
    FILL_fdtaf_target_ulong_FIELD(ts_tgid         );
    FILL_fdtaf_target_ulong_FIELD(ts_group_leader );
    FILL_fdtaf_target_ulong_FIELD(ts_thread_group );
    FILL_fdtaf_target_ulong_FIELD(ts_real_parent  );
    FILL_fdtaf_target_ulong_FIELD(ts_mm           );
    FILL_fdtaf_target_ulong_FIELD(ts_stack        );
    FILL_fdtaf_target_ulong_FIELD(ts_real_cred    );
    FILL_fdtaf_target_ulong_FIELD(ts_cred         );
    FILL_fdtaf_target_ulong_FIELD(ts_comm         );

	FILL_fdtaf_target_ulong_FIELD(modules         );
	FILL_fdtaf_target_ulong_FIELD(module_name     );
	FILL_fdtaf_target_ulong_FIELD(module_init     );
	FILL_fdtaf_target_ulong_FIELD(module_size     );
	FILL_fdtaf_target_ulong_FIELD(module_list     );
			    
    FILL_fdtaf_target_ulong_FIELD(cred_uid        );
    FILL_fdtaf_target_ulong_FIELD(cred_gid        );
    FILL_fdtaf_target_ulong_FIELD(cred_euid       );
    FILL_fdtaf_target_ulong_FIELD(cred_egid       );
    FILL_fdtaf_target_ulong_FIELD(mm_mmap         );
    FILL_fdtaf_target_ulong_FIELD(mm_pgd          );
    FILL_fdtaf_target_ulong_FIELD(mm_arg_start    );
    FILL_fdtaf_target_ulong_FIELD(mm_start_brk    );
    FILL_fdtaf_target_ulong_FIELD(mm_brk          );
    FILL_fdtaf_target_ulong_FIELD(mm_start_stack  );
    FILL_fdtaf_target_ulong_FIELD(vma_vm_start    );
    FILL_fdtaf_target_ulong_FIELD(vma_vm_end      );
    FILL_fdtaf_target_ulong_FIELD(vma_vm_next     );
    FILL_fdtaf_target_ulong_FIELD(vma_vm_file     );
    FILL_fdtaf_target_ulong_FIELD(vma_vm_flags    );
    FILL_fdtaf_target_ulong_FIELD(vma_vm_pgoff    );
    FILL_fdtaf_target_ulong_FIELD(file_dentry     );
	FILL_fdtaf_target_ulong_FIELD(file_inode		);
    FILL_fdtaf_target_ulong_FIELD(dentry_d_name   );
    FILL_fdtaf_target_ulong_FIELD(dentry_d_iname  );
    FILL_fdtaf_target_ulong_FIELD(dentry_d_parent );
    FILL_fdtaf_target_ulong_FIELD(ti_task         );
	FILL_fdtaf_target_ulong_FIELD(inode_ino);

	FILL_fdtaf_target_ulong_FIELD(proc_fork_connector);
	FILL_fdtaf_target_ulong_FIELD(proc_exit_connector);
	FILL_fdtaf_target_ulong_FIELD(proc_exec_connector);
	FILL_fdtaf_target_ulong_FIELD(vma_link);
	FILL_fdtaf_target_ulong_FIELD(remove_vma);
	FILL_fdtaf_target_ulong_FIELD(vma_adjust);

	FILL_fdtaf_target_ulong_FIELD(trim_init_extable);
#ifdef TARGET_MIPS
    FILL_fdtaf_target_ulong_FIELD(mips_pgd_current);
#endif
}

// find the corresponding section for the current os and return the section number
int find_match_section(const boost::property_tree::ptree &pt, fdtaf_target_ulong tulInitTaskAddr)
{
    ...
    for(int i = 1; i<=cntSection; ++i)
    {
        ...
        fdtaf_target_ulong tulAddr = pt.get<fdtaf_target_ulong>(sSectionNum + ".init_task_addr");
...
private:
    ...
    void load_cur_section()
    {
        ...
        BOOST_FOREACH(boost::property_tree::ptree::value_type &v, *m_cur_section)
        {
            ...
            // insert function
            // fdtaf_target_ulong addr = m_cur_section->get<fdtaf_target_ulong>(v.first);
...
const string LibraryLoader::LIBPATH_PROPERTY_NAME = "fdtaf_conf_libpath";
