...
int vmi_list_modules(CPUState* cs, uint32_t pid)
{
	...
	unordered_map<fdtaf_target_ulong, module *>::iterator iter2;
	process *proc = iter->second;
	...
	map< fdtaf_target_ulong, module *> modules;
	map< fdtaf_target_ulong, module *>::iterator iter_m;
	...
	for (iter_m = modules.begin(); iter_m!=modules.end(); iter_m++) {
		...
		fdtaf_target_ulong base = iter_m->first;
