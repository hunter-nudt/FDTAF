...
#include "shared/fdtaf-linux-procinfo.h"
...
void print_loaded_modules(CPUState *cs)
{
    ...
    fdtaf_target_ulong module_size, first_module;
    fdtaf_target_ulong next_module = OFFSET_PROFILE.modules;

	next_module -= OFFSET_PROFILE.module_list;
	first_module = next_module;

	fdtaf_read_ptr(cs, next_module + OFFSET_PROFILE.module_list, &next_module);
    ...
    while(true) {
        fdtaf_read_mem(cs, next_module + OFFSET_PROFILE.module_name, MAX_PARAM_PREFIX_LEN, module_name);
        module_name[MAX_PARAM_PREFIX_LEN - 1] = '\0';		
        fdtaf_read_ptr(cs, next_module + OFFSET_PROFILE.module_size, &module_size);
...
{
	fdtaf_target_ulong module_size, first_module, module_base;
    fdtaf_target_ulong next_module = OFFSET_PROFILE.modules;
    ...
    while(true)
    {
		...
		if(!vmi_find_module_by_key(module_name)) {								
	        fdtaf_read_ptr(cs, next_module + OFFSET_PROFILE.module_size, &module_size);
	        fdtaf_read_ptr(cs, next_module + OFFSET_PROFILE.module_init, &module_base);
...
void new_module_callback(fdtaf_callback_params* params)
{
	CPUState *cs = params->bb.cs;
	fdtaf_target_ulong pc = fdtaf_get_pc(cs);
...
void traverse_task_struct_add(CPUState *cs)
{
    ...
    fdtaf_target_ulong next_task, mm, proc_pgd, task_pgd, ts_parent_pid, ts_real_parent;
    next_task = OFFSET_PROFILE.init_task_addr;

    for (int count = MAX_LOOP_COUNT; count > 0; --count)
    {
        BREAK_IF(fdtaf_read_ptr(cs, next_task + (OFFSET_PROFILE.ts_tasks + sizeof(target_ptr)), &next_task) < 0);
        next_task -= OFFSET_PROFILE.ts_tasks;
        if(OFFSET_PROFILE.init_task_addr == next_task) {
            break;
        }

        BREAK_IF(fdtaf_read_ptr(cs, next_task + OFFSET_PROFILE.ts_mm, &mm) < 0);

        if (mm != 0) {
            BREAK_IF(fdtaf_read_ptr(cs, mm + OFFSET_PROFILE.mm_pgd, &task_pgd) < 0);

            proc_pgd = fdtaf_get_phys_addr(cs, task_pgd);
        }
        else {
        ...
        if (!vmi_find_process_by_pgd(proc_pgd)) {
            ...
            // get task_pid
            BREAK_IF(fdtaf_read_ptr(cs, next_task + OFFSET_PROFILE.ts_tgid, &task_pid) < 0);

            // get parent task's base address
            BREAK_IF(fdtaf_read_ptr(cs, next_task + OFFSET_PROFILE.ts_real_parent, &ts_real_parent) < 0
                     || fdtaf_read_ptr(cs, ts_real_parent + OFFSET_PROFILE.ts_tgid, &ts_parent_pid) < 0);
            ...
            BREAK_IF(fdtaf_read_mem(cs, next_task + OFFSET_PROFILE.ts_comm, SIZEOF_COMM, pe->name) < 0);
...
{
    unordered_set<fdtaf_target_ulong> pids;
    uint32_t task_pid = 0;
    process *right_proc = NULL;
    uint32_t right_pid = 0;

    fdtaf_target_ulong next_task, mm;
    next_task = OFFSET_PROFILE.init_task_addr;
    ...
    while(true)
    {
        ...
        BREAK_IF(fdtaf_read_ptr(cs, next_task + OFFSET_PROFILE.ts_mm, &mm) < 0);
        if (mm != 0) {
            fdtaf_read_ptr(cs, next_task + OFFSET_PROFILE.ts_tgid,
                           &task_pid);
    ...
    for(unordered_map < fdtaf_target_ulong, process * >::iterator iter = process_pid_map.begin(); iter != process_pid_map.end(); ++iter) {
...
void traverse_mmap(CPUState *cs, void *opaque)
{
    ...
    fdtaf_target_ulong mm, vma_curr, vma_file, f_dentry, f_inode, mm_mmap, vma_next = 0;
    unordered_set<fdtaf_target_ulong> module_bases;
    unsigned int inode_number;
    fdtaf_target_ulong vma_vm_start = 0, vma_vm_end = 0;
    fdtaf_target_ulong last_vm_end = 0, mod_vm_start = 0;
    char name[32];	// module file path
    string last_mod_name;
    module *mod = NULL;

    if (fdtaf_read_ptr(cs, proc->EPROC_base_addr + OFFSET_PROFILE.ts_mm, &mm) < 0)
        return;

    if (fdtaf_read_ptr(cs, mm + OFFSET_PROFILE.mm_mmap, &mm_mmap) < 0)
        return;
    ...
    while(true) {
        ...
        if (fdtaf_read_ptr(cs, vma_curr + OFFSET_PROFILE.vma_vm_start, &vma_vm_start) < 0)
            goto next;

        // read end of curr vma
        if (fdtaf_read_ptr(cs, vma_curr + OFFSET_PROFILE.vma_vm_end, &vma_vm_end) < 0)
            goto next;
        ...
        if (fdtaf_read_ptr(cs, vma_curr + OFFSET_PROFILE.vma_vm_file, &vma_file) < 0 || !vma_file)
            goto next;

        // dentry extraction from the struct* file
        if (fdtaf_read_ptr(cs, vma_file + OFFSET_PROFILE.file_dentry, &f_dentry) < 0 || !f_dentry)
            goto next;

        // read small names form the dentry
        if (fdtaf_read_mem(cs, f_dentry + OFFSET_PROFILE.dentry_d_iname, 32, name) < 0)
            goto next;

        // inode struct extraction from the struct* file
        if (fdtaf_read_ptr(cs, f_dentry + OFFSET_PROFILE.file_inode, &f_inode) < 0 || !f_inode)
            goto next;

        // inode_number extraction
        if (fdtaf_read_ptr(cs, f_inode + OFFSET_PROFILE.inode_ino ,&inode_number) < 0 || !inode_number)
            goto next;
        ...
        if (!strcmp(last_mod_name.c_str(), name)) {
            ...
            if(last_vm_end == vma_vm_start) {
                assert(mod);
                fdtaf_target_ulong new_size = vma_vm_end - mod_vm_start;
...
next:
        if (fdtaf_read_ptr(cs, vma_curr + OFFSET_PROFILE.vma_vm_next, &vma_next) < 0)
            break;
    ...
    unordered_map<fdtaf_target_ulong, module *>::iterator iter1 = proc->module_list.begin();
    unordered_set<fdtaf_target_ulong> bases_to_remove;
    ...
    unordered_set<fdtaf_target_ulong>::iterator iter2;
...
void new_proc_callback(fdtaf_callback_params* params)
{
    CPUState *cs = params->bb.cs;
    fdtaf_target_ulong pc = fdtaf_get_pc(cs);
...
void proc_end_callback(fdtaf_callback_params *params)
{
    CPUState *cs = params->bb.cs;
...
void vma_update_func_callback(fdtaf_callback_params *params)
{
    CPUState *cs = params->bb.cs;
...
void linux_tlb_call_back(fdtaf_callback_params *temp)
{
    ...
    if(first)
    {
        ...
        fdtaf_register_optimized_block_begin_callback(&vma_update_func_callback, 1, OFFSET_PROFILE.vma_adjust, OCB_CONST);
        fdtaf_register_optimized_block_begin_callback(&vma_update_func_callback, 1, OFFSET_PROFILE.vma_link, OCB_CONST);
        fdtaf_register_optimized_block_begin_callback(&vma_update_func_callback, 1, OFFSET_PROFILE.remove_vma, OCB_CONST);
        first = 0;
    }

    pgd = fdtaf_get_pgd(cs);
    proc = vmi_find_process_by_pgd(pgd);
...
{
    fdtaf_target_ulong thread_info = fdtaf_get_esp(cs) & ~(GUEST_OS_THREAD_SIZE - 1);
    static fdtaf_target_ulong last_thread_info = 0;
...
{
    fdtaf_register_optimized_block_begin_callback(&new_proc_callback, 1, OFFSET_PROFILE.proc_exec_connector, OCB_CONST);
	fdtaf_register_optimized_block_begin_callback(&new_module_callback, 1, OFFSET_PROFILE.trim_init_extable, OCB_CONST);
	fdtaf_register_optimized_block_begin_callback(&proc_end_callback, 1, OFFSET_PROFILE.proc_exit_connector, OCB_CONST);
    fdtaf_register_callback(FDTAF_TLB_EXEC_CB, linux_tlb_call_back, 1);
